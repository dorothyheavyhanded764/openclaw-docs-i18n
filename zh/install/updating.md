...
**首选**的更新路径是重新运行网站上的安装程序。它会检测现有安装，进行原地升级，并在需要时运行 `openclaw doctor`。
...
- 了解你的网关运行方式：**前台终端**还是**受监管的服务**（launchd/systemd）。
- 备份你的定制配置：
...
我们**不**推荐将 Bun 用于网关运行时（WhatsApp/Telegram 存在 bug）。要切换更新频道（git + npm 安装）：
...
使用 `--tag <dist-tag|version>` 进行一次性安装指定标签/版本。有关频道语义和发布说明，请参阅[开发频道](./development-channels.md)。注意：在 npm 安装中，网关启动时会记录更新提示（检查当前频道标签）。可通过 `update.checkOnStart: false` 禁用。

### 核心自动更新器（可选）

自动更新器**默认关闭**，是核心网关功能（非插件）。
...
控制 UI 提供**更新并重启**功能（RPC：`update.run`）。它会：
...
Doctor 是"安全更新"命令。它故意设计得简单：修复 + 迁移 + 警告。注意：如果你使用的是**源码安装**（git checkout），`openclaw doctor` 会先提示运行 `openclaw update`。它通常执行以下操作：
...
- macOS launchd（应用捆绑的 LaunchAgent）：`launchctl kickstart -k gui/$UID/ai.openclaw.gateway`（使用 `ai.openclaw.`；遗留的 `com.openclaw.*` 仍然有效）
- Linux systemd 用户服务：`systemctl --user restart openclaw-gateway[-].service`
...
运行手册 + 确切的服务标签：[网关运行手册](../gateway.md)

## 回滚 / 固定版本（当出现问题）

### 固定版本（全局安装）

安装一个已知良好的版本（将 `` 替换为最后一个正常工作的版本）：
...
- 查看：[故障排除](../gateway/troubleshooting.md)
- 在 Discord 中提问：[https://discord.gg/clawd](https://discord.gg/clawd)

[Bun（实验性）](./bun.md)[迁移指南](./migrating.md)