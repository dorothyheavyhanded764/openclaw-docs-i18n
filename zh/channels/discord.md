...
前往 [Discord 开发者门户](https://discord.com/developers/applications) 并点击 **新建应用程序**。将其命名为类似“OpenClaw”的名称。点击侧边栏的 **机器人**。将 **用户名** 设置为您为 OpenClaw 智能体（agent）起的任何名称。
...
-   默认情况下（`session.dmScope=main`），直接聊天共享智能体（agent）主会话（`agent:main:main`）。
...
注意：`off` 禁用隐式回复线程。显式的 `[[reply_to_*]]` 标签仍然有效。消息 ID 在上下文/历史记录中公开，以便智能体（agent）可以定位特定消息。
...
Discord 可以将帖子绑定到会话目标，以便该帖子中的后续消息继续路由到同一会话（包括子智能体（agent）会话）。命令：

-   `/focus ` 将当前/新帖子绑定到子智能体（agent）/会话目标
-   `/unfocus` 移除当前帖子绑定
-   `/agents` 显示活动运行和绑定状态
...
请参阅[子智能体（agent）](../tools/subagents.md)、[ACP 智能体（agent）](../tools/acp-agents.md)和[配置参考](../gateway/configuration-reference.md)。
...
有关绑定行为的详细信息，请参阅[ACP 智能体（agent）](../tools/acp-agents.md)。
...
-   智能体（agent）身份表情符号回退（`agents.list[].identity.emoji`，否则为 ”👀”）
...
使用 `eventQueue.listenerTimeout` 配置慢监听器；仅当需要为排队智能体（agent）轮次设置单独安全阀时才使用 `inboundWorker.runTimeoutMs`。
