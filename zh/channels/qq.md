...
本文介绍将 OpenClaw 与 QQ 集成，在 QQ 机器人单聊中通过自然语言与 OpenClaw 交互的完整流程。您需要在 QQ 开放平台注册并创建 QQ 机器人，获取 AppID 与 AppSecret，并在 OpenClaw 中配置 QQ 通道；若通过 Webhook 接收事件，还需在 QQ 开放平台配置请求地址与 IP 白名单。

* * *

## 快速开始

- **推荐方式**：若您的 OpenClaw 部署提供「通道配置」界面（如控制台中的通道配置），在 **QQ** 通道中填入机器人的 **App ID** 和 **App Secret** 并保存即可。
...
访问 [QQ 开放平台](https://qq.open.tencent.com/)（或腾讯 QQ 开放平台官网），使用 QQ 扫码登录开发者账号。
...
**重要：** 请妥善保管 App Secret。QQ 机器人的 AppSecret 不支持明文保存，首次查看或忘记后需重新生成，请勿泄露。
...
若您的部署带有 OpenClaw 控制台（如应用详情中的通道配置）：
...
- 确认 OpenClaw 网关正在运行，且 QQ 通道已启用。

### 鉴权失败或回调不通过

- 确认 App ID、App Secret 填写正确，无多余空格。
- 若平台有「校验令牌」或签名校验，请在 OpenClaw 配置中填写与平台一致的校验参数（参见当前版本 QQ 通道文档）。
...
[飞书](./feishu.md) | [企业微信](./wecom.md) | [Discord](./discord.md) | [Google Chat](./googlechat.md)
